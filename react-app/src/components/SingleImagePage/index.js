...
function SingleImagePage() {
    ...
    useEffect(() => {
        const errors1 = [];
        if (comment.length < 2 || !(regex1.test(comment)) || comment.length > 250) errors1.push('Comment must be at least 2 characters and less than 250 characters');
    ...
    useEffect(() => {
        const errors2 = [];
        if (comment1.length < 1 || !(regex1.test(comment1)) || comment1.length > 250) errors2.push('Comment must be at least 2 characters and less than 250 characters');
        setErrors2(errors2);
    }, [comment1])
                                                    ...
                                                    <div className='single-comment-ele'>
                                                        {
                                                            selectCommentId !== ele.id && (
                                                                <p className='commentcontent'>" {ele?.comment} "</p>
                                                            )
                                                        }
                                                        {/* <p>{ele?.comment}</p> */}

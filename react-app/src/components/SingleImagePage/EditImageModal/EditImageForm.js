...
function EditImageForm({ imageId, setShowModal }) {
    ...
    useEffect(() => {
        ...
        if (description.length < 2 || !(regex1.test(description)) || description.length > 250) errors.push('Description must be at least 2 characters and less than 250 characters');
        setErrors(errors);
    }, [content, description])
